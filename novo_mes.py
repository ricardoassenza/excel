...
def frequencia(caminho, aba_anterior, aba_atual):
    # Nomes das abas
    mes_anterior = aba_anterior
    mes_atual = aba_atual

    # Lê as duas abas do Excel
    df_antigo = pd.read_excel(caminho, sheet_name=mes_anterior)
    df_novo = pd.read_excel(caminho, sheet_name=mes_atual)
...
def finalidade(caminho, aba_anterior, aba_atual):
    # Nomes das abas
    mes_anterior = aba_anterior
    mes_atual = aba_atual
    ...
    if "Finalidade" not in df_antigo.columns:
        raise ValueError("A aba do mês anterior deve ter a coluna 'Finalidade'.")

    #  Faz o merge para trazer a frequência do mês anterior
    df_merge = df_novo.merge(
        df_antigo[["Fornecedor", "Finalidade"]],
        on="Fornecedor",
        how="left",
        suffixes=("", "_anterior")
    )
    ...
    #  Preenche a coluna Frequência (na nova planilha) com a anterior, se existir
    df_merge["Finalidade"] = df_merge["Finalidade_anterior"].combine_first(df_merge["Finalidade"])
    df_merge.drop(columns=["Finalidade_anterior"], inplace=True)

    #  Se ainda houver nulos na coluna Frequência, marca como "A classificar"
    df_merge["Finalidade"] = df_merge["Finalidade"].fillna("A definir")
